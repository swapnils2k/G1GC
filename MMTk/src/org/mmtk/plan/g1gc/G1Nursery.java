 ...
 */
@Uninterruptible
public abstract class G1Nursery extends StopTheWorld {
   ...
   *
   */
  protected static final float NURSEY_ESTIMATE     = 0.5f;
  protected static final float SURVIVAL_ESTIMATE   = 0.5f; 
  protected static final float MATURE_FRACTION     = 0.5f; 

  // Fields which are required to be declared for cross generation object reference
  ...
  protected static final VMRequest vmRequest = VMRequest.highFraction(NURSERY_VM_FRACTION);
  ...
  public boolean gcSurvivor = false;
  public boolean gcNursery = false;

  public boolean nextGCNursery = false;
  public boolean nextGCFullHeap = false;
  public boolean nextGCSurvivor = false;
  ...
  public void printPreStats() {
    if ((Options.verbose.getValue() >= 1) && (gcNursery))
      Log.write("[Nursery]");

    if ((Options.verbose.getValue() >= 1) && (gcSurvivor))
      Log.write("[Survivor]");
  ...
  public final boolean isCurrentGCNursery() {
    return !(IGNORE_REMSETS || gcFullHeap) && gcNursery;
  }

  public final boolean isCurrentGCSurvivor() {
    return !(IGNORE_REMSETS || gcFullHeap) && gcSurvivor;
  }
  ...
  public int sanityExpectedRC(ObjectReference object, int sanityRootRC) {
    ...
    // Immortal spaces
    if (space == G1Nursery.immortalSpace || space == G1Nursery.vmSpace) {
  ...
  public void fullyBooted() {
    super.fullyBooted();
    nurserySpace.setZeroingApproach(true, false);
  }
}
